...
public class CreatedAuctionsActivity extends AppCompatActivity {
    private RecyclerView recyclerView;
    private ArrayList<Auction> auctionsList;
    private ActivityAuctionAdapter adapter;
    private TextView oopsMessage;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.fragment_activity_auction_list);

        String userId = Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid();
        recyclerView = findViewById(R.id.recyclerActivityView);

        LinearLayoutManager layoutManager = new LinearLayoutManager(CreatedAuctionsActivity.this);
        recyclerView.setLayoutManager(layoutManager);

        oopsMessage = findViewById(R.id.oopsMessageCreatedAuctions);
...
//        });
        ...
        auctionsRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for(DataSnapshot auctionSnapshot : snapshot.getChildren()){
                    Auction auction = auctionSnapshot.getValue(Auction.class);
                    if(auction!= null && auction.getAuctionId().startsWith(userId)){
                        auctionsList.add(auction);
                    }
                }
